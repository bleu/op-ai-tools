    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(
            ...
            try:
                ...
                summaries.append({"url": url, "data": {"error": str(exc)}, "error": str(exc)})
...
async def upsert_summaries(summaries: List[Dict[str, str]]):
    raw_summaries = [
        ...
        for summary in summaries if summary['error'] is None
    ]
...
async def main():
    logger.logger.info("Starting parallel summarization of out-of-date forum threads...")
