...
class TopicData:
    ...
    def from_json(json_dict: dict) -> "TopicData":
        json_dict["post_stream"] = PostStream(
            posts=[Post(**post) for post in json_dict["post_stream"]["posts"]],
            stream=json_dict["post_stream"]["stream"],
        )

        if json_dict.get("accepted_answer"):
            json_dict["accepted_answer"] = AcceptedAnswer(
                **json_dict["accepted_answer"]
            )

        json_dict["actions_summary"] = [
            ActionSummary(**item) for item in json_dict["actions_summary"]
        ]

        json_dict["details"] = Details(
            can_edit=json_dict["details"]["can_edit"],
            notification_level=json_dict["details"]["notification_level"],
            participants=[
                Participant(**participant)
                for participant in json_dict["details"]["participants"]
            ],
            created_by=User(**json_dict["details"]["created_by"]),
            last_poster=User(**json_dict["details"]["last_poster"]),
            links=[]
            if "links" not in json_dict["details"]
            else [Link(**link) for link in json_dict["details"]["links"]],
        )
...
if __name__ == "__main__":
    ...
    def load_and_parse_json_files_from_directory(
        directory_path: str,
    ) -> List["TopicData"]:
        topic_data_objects = []
    ...
    def extract_primitive_fields(topic_data: TopicData, filename: str) -> dict:
        ...
        for field in fields(topic_data):
            ...
            if field.name == "accepted_answer":
                primitive_data["accepted_answer"] = (
                    getattr(topic_data, field.name) != None
                )

        return primitive_data

    def create_dataframe_from_topicdata(
        file_topicdata_pairs: List[Tuple[str, TopicData]],
    ) -> pd.DataFrame:
